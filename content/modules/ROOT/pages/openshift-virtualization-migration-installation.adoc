# Installation of Ansible for OpenShift Virtualization Migration


IMPORTANT: If you are using your machine, and doing ssh to bastion, remember to use tmux
...
== create the vms to be migrated

Log in your VMware environment using the following credentials:
...
After your login, go to the Menu and then to Inventory.

image::Ansible-image::Ansible-Migration-Install/vmware-inventory.png[link=self, window=blank, width=100%]

Them go to Virtual Machines Icon, expand the templates folder, the RHEL folder and there you should see four templates.

NOTE: On this lab you will use rhel9-tpl only.

image::Ansible-image::Ansible-Migration-Install/vmware-inventory.png[link=self, window=blank, width=100%]

On the *rhel9-tpl* use your right click and create a new VM from This template.

image::Ansible-image::Ansible-Migration-Install/create-vm.png[link=self, window=blank, width=100%]

Select the your folder: {vcenter_vm_folder}, name the vm to *rhel9-to-be-migrated-1* and click next.

image::Ansible-image::Ansible-Migration-Install/vm-folder-select.png[link=self, window=blank, width=100%]

Select one of three hosts, see the compatibility check in green and click next.

image::Ansible-image::Ansible-Migration-Install/vm-esxi.png[link=self, window=blank, width=100%]

Select one of DataStore locations, see the compatibility check in green and click next.

image::Ansible-image::Ansible-Migration-Install/vm-storage.png[link=self, window=blank, width=100%]

Mark the options *Customize this virtual machine's hardware* and *Power on virtual machine after creation* and click in next.

image::Ansible-image::Ansible-Migration-Install/vm-options.png[link=self, window=blank, width=100%]

Go to add new device, add a Network card adapter and click next.

image::Ansible-image::Ansible-Migration-Install/vm-new-device.png[link=self, window=blank, width=100%]

Finish the creation.

IMPORTANT: Repeat this process more two times and name the vms as *rhel9-to-be-migrated-2* and *rhel9-to-be-migrated-3*
...
.. Create a new Python Virtual Environment called `etx` within the `~/.virtualenvs` directory
+
...
+
NOTE: Podman is already installed on your system, on real scenario do the following command.
+
[source,shell]
----
...
----
+
For your commodity the container image already been deployed into your bastion.
+
...
quay.io/rhpds/openshift-virtualization-migration-ee-2.5  v9          724ea6e8252e  4 months ago  1.08 GB
----

=== Acquire Source Code

Source code, including Automation tooling (playbooks, collections, etc) for the Ansible for OpenShift Virtualization Migration are located within a protected collection into Red Hat Ansible Automation HUB within the link:https://console.redhat.com/ansible/automation-hub/repo/validated/infra/openshift_virtualization_migration[Red Hat Automation HUB] or https://catalog-item-assets.s3.us-east-2.amazonaws.com/infra-openshift_virtualization_migration-1.21.1.tar.gz[use this link] if you do not have login on console.redhat.com.

. Download the Ansible tarball for OpenShift Virtualization Migration Ansible Collection (`infra.openshift_virtualization_migration`) source code to your local machine and change into the cloned directory. When prompted, enter the Git username and password that were provided previously:
+
[source,shell]
----
$ wget https://catalog-item-assets.s3.us-east-2.amazonaws.com/infra-openshift_virtualization_migration-1.21.1.tar.gz

$ mkdir openshift_virtualization_migration

$ tar xf infra-openshift_virtualization_migration-1.21.1.tar.gz -C openshift_virtualization_migration

$ cd openshift_virtualization_migration
----
+
Now you need to create your git environment to receive the playbook contents to use it with Ansible Automation Platform:
+
. login into your gitea {gitea_console_url}
. user: {gitea_user}
. Password: {gitea_password}
+
IMPORTANT: If you do not have a user, you can register one with your email and password, they are not validated.
+
image::Ansible-Migration-Install/gitea-login.png[link=self, window=blank, width=100%]
+
. Create a repo called *openshift_virtualization_migration* on gitea
+
image::Ansible-Migration-Install/gitea-create-repo.png[link=self, window=blank, width=100%]
+
image::Ansible-Migration-Install/gitea-create-repo-II.png[link=self, window=blank, width=100%]
+
image::Ansible-Migration-Install/gitea-create-repo-III.png[link=self, window=blank, width=100%]
+
GUID: {GUID}
+
[source,console,subs="attributes",role=execute]
...
git remote add origin https://gitea.apps.{openshift_cluster_ingress_domain}/<YOUR GIT USER>/openshift_virtualization_migration.git
git push -u origin main
----

=== Setting Environment Variables

IMPORTANT: The complete respository zip file is already deployed into your home area.
...
. First set a new environment variable called `OPENSHIFT_VIRTUALIZATION_MIGRATION_REPO` to be the current working directory which should be the root of the your Ansible for OpenShift Virtualization Migration Git repository.
+
[source,shell]
----
...
[source,console,subs="attributes",role=execute]
----
$ git clone https://github.com/rhpds/etx-virt-vma-factory-showroom etx-virt_delivery
----
+
.Sample Output
[source,text]
...
[TIP]
====
The *ETX Workshop Git Repository* is the repository located at link:https://github.com/rhpds/etx-virt-vma-factory-showroom[https://github.com/rhpds/etx-virt-vma-factory-showroom] which will used again in the future exercise relating to xref:VMs-and-gitops.adoc[GitOps and Virtual Machines].
====
[IMPORTANT]
====
...
----
+
There are several methods for which Ansible Automation Platform can be subscribed, including using a Service Account from the Red Hat Hybrid Cloud Console or a Subscription manifest. For this workshop, you will make use of a Subscription manifest that provides the necessary entitlements for the target Ansible Automation Platform.
+
NOTE: There is a manifest on the base of directory of $EXT_REPO
+
If you're *not* using the bastion machine provided by the RHDP deployment as your instance, download the manifest file provided to the root of your OpenShift Virtualization Migration Git repository.
+
[source,console,subs="attributes",role=execute]
----
curl -L -o $OPENSHIFT_VIRTUALIZATION_MIGRATION_REPO/aap-manifest.zip https://github.com/rhpds/etx-virt-vma-factory-showroom/blob/main/aap-manifests.zip
----
...
link:https://github.com/rhpds/openshift-virtualization-vmware-vddk[VDDK]
===

you can point to quay.io/andharri/vddk on your provider to obatin the vddk container instead to build one
...
Copy the aap manifest into the working directory

[source,shell]
----
cp $ETX_REPO/aap-manifests.zip $OPENSHIFT_VIRTUALIZATION_MIGRATION_REPO
----

ensure that you are on your correct work directory:
...
ansible-navigator run \
  --eei=quay.io/rhpds/openshift-virtualization-migration-ee-2.5:v9 \
  -m stdout \
  --pp=missing \
  --eev=$OPENSHIFT_VIRTUALIZATION_MIGRATION_REPO:/runner/project:Z \
  --eev=$OPENSHIFT_VIRTUALIZATION_MIGRATION_REPO:/usr/share/ansible/collections/ansible_collections/infra/openshift_virtualization_migration:Z \
